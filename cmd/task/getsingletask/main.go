...
func handler(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
  creatorId := req.PathParameters["creator_id"]
	...
	newTasks, err := taskService.GetSingleTask(creatorId,projectId,taskId)
	
	if err != nil {
        fmt.Print(newTasks)
		logger.Error("error getting the task: " + err.Error())
		return response.LambdaErrorResponse(nil, "Error in fetching the task", 500, 500), nil
	}

	if len(newTasks) == 0 {
		logger.Info("No task assigned")
		
		return response.LambdaErrorResponse(nil, "invalid task id", 404, 404), nil
	}
