...
func handler(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	empId := req.PathParameters["manager_id"]

	

	newTasks, err := taskService.GetAllManagerTask(empId)
	
	if err != nil {
        fmt.Print(newTasks)
		logger.Error("error getting the tasks: " + err.Error())
		return response.LambdaErrorResponse(nil, "Error in fetching the tasks"+err.Error(), 500, 500), nil
	}
